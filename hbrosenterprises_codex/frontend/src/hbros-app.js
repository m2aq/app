...
function saveWorkers(w) { localStorage.setItem('hb_workers', JSON.stringify(w)); }
...
function saveFields(f) { localStorage.setItem('hb_fields', JSON.stringify(f)); }
        ...
        const marker = L.marker(pt, {
            ...
            icon: L.divIcon({
                ...
                html: '<div style="position:relative;width:28px;height:34px;">' +
                    '<div style="position:absolute;left:50%;top:3px;transform:translateX(-50%);width:20px;height:20px;border-radius:9999px;background:#3b82f6;border:2px solid #1d4ed8;box-shadow:0 2px 6px rgba(0,0,0,.25);"></div>' +
                    '<div style="position:absolute;left:50%;top:22px;transform:translateX(-50%);width:0;height:0;border-left:6px solid transparent;border-right:6px solid transparent;border-top:10px solid #1d4ed8;"></div>' +
                    '</div>',
                iconSize: [28, 34],
                iconAnchor: [14, 32]
            })
            ...
            <div id="field-card-${field.id}" class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all">
                ...
                <div class="px-4 pb-4">
                    <div class="bg-gray-50 rounded-lg p-3 text-xs">
                        <a href="https://www.google.com/maps?q=${centerLat.toFixed(4)},${centerLng.toFixed(4)}" target="_blank" class="text-blue-600 hover:text-blue-700">View on Map</a>
                    </div>
                </div>
