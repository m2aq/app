...
const DEFAULT_CLOUD_CONFIG = {
    url: 'https://cfmfefiwlzfeaueliiog.supabase.co',
    anonKey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNmbWZlZml3bHpmZWF1ZWxpaW9nIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzYyMDc0MzUsImV4cCI6MjA5MTc4MzQzNX0.H3pu_tHa0tKAvvQ9Ag0HOAKr0bDGbANi77-6t3bWpPA',
    enabled: true
};
...
function loadCloudConfig() {
    try {
        ...
        return {
            url: cfg.url || DEFAULT_CLOUD_CONFIG.url,
            anonKey: cfg.anonKey || DEFAULT_CLOUD_CONFIG.anonKey,
            enabled: typeof cfg.enabled === 'boolean' ? cfg.enabled : DEFAULT_CLOUD_CONFIG.enabled
        };
    } catch {
...
function cloudHeaders() {
    ...
    // New publishable keys (sb_publishable_*) should not be forced as Bearer JWT.
    const isJwtLike = typeof cfg.anonKey === 'string' && cfg.anonKey.split('.').length === 3;
    if (isJwtLike) headers.Authorization = `Bearer ${cfg.anonKey}`;
    return headers;
}
    ...
    document.getElementById('workers-list').innerHTML = workers.map(w => `
        <div class="bg-gray-50 rounded-lg p-3 flex items-center justify-between">
            ...
            <div class="edit-only-flex items-center gap-2">
                <button onclick="renameWorker('${w.id}')" class="text-blue-600 hover:text-blue-800 text-xs font-semibold">Edit</button>
                <button onclick="removeWorker('${w.id}')" class="text-red-500 hover:text-red-700 text-sm">✕</button>
            </div>
        </div>
    `).join('');
}
...
function renameWorker(id) {
    ...
    const currentName = workers[idx].name || '';
    const nextName = prompt('Edit worker name:', currentName);
    ...
    workers[idx].name = cleanName;
    saveWorkers(workers);
    renderAll();
}
window.renameWorker = renameWorker;
...
function initCloudUI() {
    if (!localStorage.getItem(CLOUD_CFG_KEY)) {
